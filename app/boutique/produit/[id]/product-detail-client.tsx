...
export function ProductDetailClient({
  ...
  useEffect(() => {
    async function fetchVariants() {
      try {
        const response = await fetch(`/api/products/${product.id}/variants?locale=fr`);
        if (response.ok) {
  ...
  const handleAddToCart = () => {
    ...
    toast.success("Ajouté au panier", `${product.name} - ${selectedVariant.name}`);
  };
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-warm-50 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-warm-500">3-5 jours ouvrés</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-warm-500">100% satisfait</p>
                </div>
              </div>
            </div>
          </motion.div>
